<template>
  <div class="music" :style="{ background: `url(${bgurl}) no-repeat` }">
    <GithubCorner />
...
import { TweenMax, TimelineMax, TweenLite, Power2, Power3 } from "gsap";

// 一曲相思-阿悠悠
// Cashmere Cat,Lana Del Rey - National Anthem (Cashmere Cat Remix)
// Hawk Nelson - Sold Out
// lol
// Lana Del Rey - Body Electric (Kraeday Trap Remix)
// Naughty Boy,Sam Smith - La La La
import URL from "../music-files/Hawk Nelson - Sold Out.mp3";
import bgurl from "../assets/5f1a7c9a3b617.jpg";
...
// const URL = "../assets/Lana Del Rey - Body Electric (Kraeday Trap Remix).mp3";
...
function ParticleBufferGeometry(options) {
  ...
  if (typeof options.particleGeometry == "function") {
    ...
    t = function () {
      return options.particleGeometry[
        THREE.Math.randInt(0, options.particleGeometry.length - 1)
      ];
    };
  } else {
    t = function () {
  ...
  for (
    var i = [], n = [], r = [], o = [], a = 0;
    a < options.particleCount;
    a++
  ) {
    for (
      var s = t(), l = s.indexArray.length, h = o.length / 3, u = 0;
      u < l;
      u++
    )
...
function onUpdate() {
  ...
  audioUniforms.smoothedVolume.value +=
    0.1 * (e - audioUniforms.smoothedVolume.value);
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      document.querySelector("#play-btn").style.display = "block";
      var bridge = renderWebGL(document.getElementById("play-content"), {
        audioSrc: URL,
      });
